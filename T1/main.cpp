...
static void key_event(GLFWwindow* window, int key, int scancode, int action, int mods){
    ...
    if(key==65) t_x -= 0.01; // tecla para esquerda 

    if(key==87) t_y += 0.01; // tecla para cima
...
int main(void){
    ...
    // Associando nossa janela com eventos do mouse e teclado
    
    glfwSetKeyCallback(window, key_event); // teclado

    //glfwSetMouseButtonCallback(window, mouse_event); //mouse
